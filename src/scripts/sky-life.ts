...
const EVENTS: EventSpec[] = [
     ...
     hero, which makes this the one moment a flock can have to itself. */
  {
    ...
    build: (el) => {
      ...
      tl.set(el, { x: vw(108), y, opacity: 0 })
        ...
        .to(el, { y: y + vh(4.5), duration: 15.5, ease: 'sine.inOut' }, 0)
     ...
     hero. The jet gets the whole of it. */
  {
    ...
    build: (el) => {
      ...
      tl.set(el, { x: -vw(16), y: vh(14), opacity: 0, rotation: 0 })
        ...
        .to(trail, { scaleX: 1, duration: 7.5, ease: 'power1.out' }, 0.5)
     ...
     one thing this layer must not become. */
  {
    ...
    build: (el) => {
      ...
      tl.set(el, { x: -vw(16), y, opacity: 0, scaleX: -1 })
        ...
        .to(el, { y: y - vh(3.2), duration: 11.5, ease: 'sine.inOut' }, 0)
     ...
     the system assembles behind it. */
  {
    ...
    build: (el) => {
      ...
      tl.set(el, { x: vw(13), y: vh(104), opacity: 0, scale: 1, rotation: 4 })
        .set(plume, { scaleY: 0 })
        ...
        .to(el, { x: vw(21), rotation: 15, duration: 7.4, ease: 'power2.in' }, 0)
        ...
        .to(plume, { scaleY: 1, duration: 4.2, ease: 'power1.out' }, 0.25)
...
function reactor(): Reactor | null {
  ...
  return {
    bubble(text, cx, cy) {
      ...
      bubbleTl = gsap
        .timeline()
        .set(bubbleEl, { x: bx, y: by, opacity: 0, scale: 0.86, rotation: -2 })
        .to(bubbleEl, {
          opacity: 1,
          scale: 1,
          rotation: 0,
        ...
        .to(bubbleEl, { y: by - 9, duration: 1.5, ease: 'sine.out' }, 0.42)
        .to(bubbleEl, { opacity: 0, scale: 0.96, duration: 0.34, ease: 'power2.in' }, 1.62);
    },
    ...
    puff(cx, cy, opt) {
      ...
      for (let i = 0; i < puffs.length; i++) {
        ...
        const push = opt.bias === undefined ? 1 : 1 + Math.cos(a - opt.bias) * 0.55;
        ...
        gsap
          ...
          .to(el, { scale: s, opacity: 0.95, duration: dur * 0.26, ease: 'power2.out' })
          .to(
            el,
            {
              x: x + Math.cos(a) * d,
          ...
          .to(el, { opacity: 0, duration: dur * 0.62, ease: 'power1.in' }, dur * 0.38);
      }
    },
    ...
    ring(cx, cy) {
      ...
      gsap
        ...
        .to(ringEl, { opacity: 0, duration: 0.5, ease: 'power1.in' }, 0.12);
    },
...
const BIRD_LINE = 'EY. CON LOS PÁJAROS NO.';
...
export function skyLife(activity: Activity): SkyLife {
  ...
  const fx = reactor();
  let running: { tl: gsap.core.Timeline; el: HTMLElement; id: string; used: boolean } | null = null;
  ...
  const finish = (el: HTMLElement): void => {
    ...
    gsap.set(el.querySelectorAll('[data-bird], [data-jet-craft]'), { clearProps: 'transform' });
    if (running?.el === el) {
      running = null;
      busy(false);
    }
  ...
  const retire = (r: { tl: gsap.core.Timeline; el: HTMLElement }): void => {
    ...
    gsap.to(r.el, {
      ...
      ease: 'power1.in',
      onComplete: () => {
        gsap.set(r.el.querySelectorAll('[data-bird], [data-jet-craft]'), {
  ...
  const react = (e: MouseEvent): void => {
    ...
    const target = (e.target as HTMLElement | null)?.closest<HTMLElement>('.hit');
    ...
    if (kind === 'birds') {
      ...
      const birds = Array.from(running.el.querySelectorAll<HTMLElement>('[data-bird]'));
      ...
      gsap.to(running.tl, { timeScale: 2.6, duration: 0.55, ease: 'power2.in' });
    } else if (kind === 'jet') {
      ...
      fx.puff(cx, cy - 16, { count: 8, reach: 62, bias: -Math.PI / 2, scale: 1.3, duration: 0.9 });
      ...
      gsap
        .timeline({ onComplete: () => finish(el) })
        // Forward momentum, bleeding off. It does not stop dead.
        .to(el, { x: x0 + vw(23), duration: 3.1, ease: 'power1.out' }, 0.12)
        // The fall, accelerating: half a parabola, not a dive.
        .to(el, { y: vh(96), duration: 3.1, ease: 'power2.in' }, 0.12)
        // Nose up first — the stall — then over and down. 34°, no more:
        // past about 50 it stops being funny and starts being a crash.
        .to(craft, { rotation: -11, duration: 0.42, ease: 'power2.out' }, 0.12)
        .to(craft, { rotation: 34, duration: 2.4, ease: 'power2.inOut' }, 0.6)
        // The contrail stops being produced and comes apart.
        .to(trail, { opacity: 0, scaleX: 0.6, duration: 1, ease: 'power2.in' }, 0.12)
      ...
      fx.ring(cx, cy);
      fx.puff(cx, cy, { count: 8, reach: 54, scale: 1.05, duration: 0.8 });
      const plume = running.el.querySelector<HTMLElement>('[data-launch-plume]');
      if (plume) {
        gsap
          .timeline()
          .to(plume, { scaleX: 1.5, opacity: 1, duration: 0.16, ease: 'power2.out' })
          .to(plume, { scaleX: 1, duration: 0.9, ease: 'power2.out' });
      }
      gsap.to(running.tl, { timeScale: 3.6, duration: 0.42, ease: 'power2.in' });
    }
